...
def setup(use_petsc=0,kernel_language='Python',outdir='./_output1',solver_type='classic',disable_output=False):
    ...
    if kernel_language == 'Python': 
        ...
        riemann_solver = riemann.Mu_I.Full_ad_model
    ...
    state.aux[0,:] = phim0 - c*np.exp(-((xc-0.5)*d)**2)            #phi_m
    state.aux[1,:] = 2*c*d**2*np.exp(-((xc-0.5)*d)**2)*(xc-.5) #phi_mz
    #state.aux[0,:] = 1                                          #phi_m
